...
class InfLoRA(BaseLearner):
    ...
    def _train(self, train_loader, test_loader):
        ...
        with torch.no_grad():
            ...
            # Ensure we always operate on numpy arrays to avoid torch.transpose() issues
            self.feature_mat = []
            for p in range(len(self.feature_list)):
                ...
                # feat_np: [d, r], build projection matrix U U^T
                Uf_np = feat_np @ feat_np.T
                Uf = torch.from_numpy(Uf_np.astype(np.float32))
    ...
    def update_DualGPM(self, mat_list):
        ...
        if len(self.feature_list) == 0:
            ...
            for i in range(len(mat_list)):
                # Ensure feature_list entries are numpy arrays
                feat = self.feature_list[i]
                if isinstance(feat, torch.Tensor):
                    feat = feat.detach().cpu().numpy()
                else:
                    feat = np.asarray(feat)
                self.feature_list[i] = feat
                ...
                if self.project_type[i] == "remove":
                    ...
                    sval_total = (S1**2).sum()
                    # Projected Representation (Eq-8)
                    act_hat = np.dot(np.dot(feat, feat.T), activation)
                    U, S, Vh = np.linalg.svd(act_hat, full_matrices=False)
                    # criteria (Eq-9)
                    sval_hat = (S**2).sum()
                    sval_ratio = (S**2) / sval_total
                    ...
                    act_feature = feat - np.dot(np.dot(U[:, 0:r], U[:, 0:r].T), feat)
                    Ui, Si, Vi = np.linalg.svd(act_feature)
                    self.feature_list[i] = Ui[:, : feat.shape[1] - r]
