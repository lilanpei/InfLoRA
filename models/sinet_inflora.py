...
def _create_vision_transformer(variant, pretrained=False, **kwargs):
    ...
    # Handle both old dict-style and new PretrainedCfg object style
    if hasattr(pretrained_cfg, "num_classes"):
    ...
    # Note: pretrained_custom_load removed for newer timm compatibility
...
class SiNet(nn.Module):
    ...
    def __init__(self, args):
        ...
        # Use local checkpoint to avoid network download on compute nodes
        local_ckpt = os.path.expanduser("~/vit_b16_in21k.pth")
        use_pretrained = not os.path.exists(local_ckpt)
        self.image_encoder = _create_vision_transformer(
            "vit_base_patch16_224_in21k", pretrained=use_pretrained, **model_kwargs
        )
        # Load from local checkpoint if available
        if os.path.exists(local_ckpt):
            print(f"Loading ViT weights from local checkpoint: {local_ckpt}")
            state_dict = torch.load(local_ckpt, map_location="cpu")
            # Filter out incompatible keys (LoRA layers won't be in pretrained)
            model_state = self.image_encoder.state_dict()
            filtered_state = {
                k: v
            ...
            print(
                f"Loaded {len(filtered_state)} keys, missing {len(missing)}, unexpected {len(unexpected)}"
            )
